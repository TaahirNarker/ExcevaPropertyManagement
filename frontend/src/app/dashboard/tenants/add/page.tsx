  ...
  const handleFormSubmit = async (data: TenantFormData) => {
    try {
      ...
      const tenantData = {
        ...
        // Essential tenant fields only (matching backend model exactly)
        tenant_code: '', // Will be auto-generated by backend
        name: `${data.first_name} ${data.last_name}`, // Frontend expects this, backend will handle it
      ...
      const requiredFields = ['email', 'first_name', 'last_name', 'id_number', 'phone', 'address', 'city', 'province', 'postal_code'];
      const missingFields = requiredFields.filter(field => !tenantData[field as keyof typeof tenantData]);
      ...
      console.log('Data type check:', {
        email: typeof tenantData.email,
        first_name: typeof tenantData.first_name,
        last_name: typeof tenantData.last_name,
        id_number: typeof tenantData.id_number,
        date_of_birth: typeof tenantData.date_of_birth,
        phone: typeof tenantData.phone,
        address: typeof tenantData.address,
        city: typeof tenantData.city,
        province: typeof tenantData.province,
        postal_code: typeof tenantData.postal_code,
        employment_status: typeof tenantData.employment_status,
      ...
      if (error.message) {
        ...
        if (typeof errorData === 'string') {
          toast.error(`Failed to create tenant: ${errorData}`);
        } else if (errorData.detail) {
          toast.error(`Failed to create tenant: ${errorData.detail}`);
        } else if (errorData.non_field_errors) {
          toast.error(`Failed to create tenant: ${errorData.non_field_errors.join(', ')}`);
        } else {
          ...
          toast.error(`Failed to create tenant: ${fieldErrors}`);
        }
      } else {
