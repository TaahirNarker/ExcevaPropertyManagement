...
export default function BurgerMenu() {
  ...
  return (
    <div className="relative" style={{ zIndex: 99999 }}>
      <button
        onClick={toggleMenu}
        className="p-2 rounded-lg bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 transition-colors duration-200"
      ...
      {/* Optimized menu without framer-motion */}
      {isOpen && (
        <>
          {/* Backdrop overlay */}
          <div
            className={`fixed inset-0 ${isDark ? 'bg-black' : 'bg-gray-600'} opacity-50 transition-opacity duration-200`}
            style={{ zIndex: 99990 }}
            onClick={toggleMenu}
          />
          
          <div
            className={`fixed left-0 top-16 mt-2 w-64 rounded-lg shadow-xl border overflow-hidden transition-colors duration-300 ${
              isDark 
            ...
            <div className="p-4 space-y-4">
              {menuItems.map((section, sectionIndex) => (
                <div key={sectionIndex} className="space-y-2">
                  ...
                  {/* Collapsible section - using CSS transitions instead of framer-motion */}
                  {activeSection === sectionIndex && (
                    <div className="pl-4 space-y-2 transition-colors duration-300">
                          ...
                          {hasSubItems(item) ? (
                            ...
                            <>
                              <button
                                ...
                                className={`w-full text-left px-4 py-2 rounded-lg transition-colors duration-200 font-medium text-sm ${
                                  isDark
